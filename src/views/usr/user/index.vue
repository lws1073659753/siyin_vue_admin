<template>
  <div class="app-container">
    <div class="block">
      ...
      <el-row>
        <el-col :span="24">
          ...
          <el-button v-permission="['/user/freeze']" type="success" size="mini" icon="el-icon-delete"
            @click.native="changeUserStatusBatch(1)">
            {{ $t('button.enable') }}
          </el-button>
          <el-button v-permission="['/user/freeze']" type="danger" size="mini" icon="el-icon-delete"
            @click.native="changeUserStatusBatch(0)">
            {{ $t('button.disable') }}
          </el-button>
        </el-col>
      </el-row>
    </div>
    <el-table v-loading="listLoading" :stripe="true" :data="list" row-key="id" element-loading-text="Loading" border fit
    ...
    highlight-current-row @current-change="handleCurrentChange" @selection-change="handleSelectionChange">
      ...
      <el-table-column fixed="right" label="操作" width="400" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button v-permission="['/user/edit']" type="primary" size="mini" @click="edit(row)">{{ $t('button.edit') }}
          </el-button>
          <el-button v-permission="['/user/setRole']" type="success" size="mini" @click="openRole(row)">{{
              $t('button.role')
          }}</el-button>
          <el-button v-permission="['/user/reset']" type="info" size="mini" @click="resetPwd(row)">{{
              $t('button.reset')
          }}</el-button>
          <el-button v-permission="['/user/delete']" type="danger" size="mini" icon="el-icon-delete"
            @click.native="remove(row)">{{ $t('button.delete') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination background layout="total, sizes, prev, pager, next, jumper" :page-sizes="[5, 10, 20, 50, 100, 500]"
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="钉钉号" >
              <el-input v-model="form.dingDing" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
    ...
    <el-dialog title="角色分配" :visible.sync="roleDialog.visible" width="50%">
      <el-form>
        <el-row>
          <el-col :span="12">
            <el-tree ref="roleTree" :data="roleDialog.roles" show-checkbox node-key="id"
              :default-checked-keys="roleDialog.checkedRoleKeys" :props="roleDialog.defaultProps" />
